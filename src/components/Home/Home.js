...
export const Home = () => {
  ...
  const getShoes = (id) => {
    return shoesJson.find((shoes) => shoes.id === id);
  };

  const getListForRange = (init, end) => {
    return shoesJson.slice(init, end);
  };

  return (
    <div>
      <ProductView shoes={getShoes(1)} />
      <h2 className="mx-14 my-3 text-lg p-1 font-serif font-extrabold bg-herring-silver">
        COMPLETA TU LOOK
      </h2>
      <section className="mx-14 max-w-full flex justify-center items-center overflow-x-hidden">
        {getListForRange(0, 4).map((shoes, index) => (
          <Product key={index} shoes={shoes} />
        ))}
      </section>
      <h2 className="mx-14 my-3 text-lg p-1 border-b font-serif font-bold text-paris-paving">
        PRODUCTOS RECOMENDADOS
      </h2>
      <section className="mx-14 mb-7 max-w-full flex justify-center items-center overflow-x-hidden">
        {getListForRange(4, 8).map((shoes, index) => (
          <Product key={index} shoes={shoes} />
        ))}
      </section>
